...
export const WeeklySpendingReport = () => {
  ...
  // Get last week's spending for comparison
      ...
      <div className="bg-muted/50 rounded-2xl p-4">
        <div className="flex items-center justify-between">
          ...
          {lastWeekSpending > 0 && (
            <div className={`flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium ${
              isSpendingUp ? 'bg-warning-soft text-warning' : 'bg-success-soft text-success'
            }`}>
              {isSpendingUp ? (
        ...
        {lastWeekSpending > 0 && (
          <p className="text-xs text-muted-foreground mt-1">
            {isSpendingUp ? 'More than' : 'Less than'} last week
          </p>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-2">
          {topCategories.map(([cat, amount]) => {
            ...
            const percent = Math.round((amount / weeklySpending) * 100);
            return (
