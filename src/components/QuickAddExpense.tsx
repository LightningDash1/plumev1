...
export const QuickAddExpense = ({ onExpenseAdded, addTransaction }: QuickAddExpenseProps) => {
  ...
  return (
    <>
        ...
        <DialogContent className="sm:max-w-md mx-4 rounded-3xl">
          ...
          <div className="space-y-5 py-4">
            <div className="space-y-2">
              <Label htmlFor="amount" className="text-sm font-semibold text-muted-foreground">Amount</Label>
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-2xl font-bold text-muted-foreground">₹</span>
                <Input id="amount" type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0" className="pl-10 text-3xl font-bold h-16 rounded-2xl border-2 focus:border-primary" autoFocus />
              </div>
            </div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {categories.map((cat) => (
                  <button key={cat} onClick={() => setCategory(cat)}
                    className={cn("flex flex-col items-center p-3 rounded-xl transition-all",
                      category === cat ? "bg-primary text-primary-foreground scale-105" : "bg-secondary hover:bg-secondary/80"
                    )}>
                    <span className="text-xl mb-1">{categoryInfo[cat].emoji}</span>
                    <span className="text-xs font-medium truncate w-full text-center">{categoryInfo[cat].label.split(' ')[0]}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              {suggestedCategory && (
                <button onClick={applySuggestion} className="flex items-center gap-2 px-3 py-2 bg-primary-soft rounded-xl text-sm animate-fade-in">
                  ...
                  <span className="text-foreground">Suggest: <strong>{categoryInfo[suggestedCategory].label}</strong></span>
            ...
            <div className="flex items-center justify-between p-4 bg-secondary rounded-2xl">
              <div>
                <p className="font-semibold text-foreground">{isWant ? 'Want 🎮' : 'Need 🎯'}</p>
                <p className="text-sm text-muted-foreground">{isWant ? 'Nice to have' : 'Essential expense'}</p>
              </div>
      ...
      {showImpactPreview && (
        <SpendingImpactPreview transactionAmount={lastTransactionAmount} onClose={handleImpactClose} />
      )}
    </>
  );
};
